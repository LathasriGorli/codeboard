...
const appointScreenData = [
  {
    icon: <MotherIcon className="w-4 h-4" />,
    title: "Patient",
      ...
      <Card className="w-[calc(100vw-35rem)] h-auto flex rounded-md bg-(--an-appoint-screen-background) shadow-none border-none py-3 px-0">
        <CardHeader className="flex justify-between items-center px-4">
          ...
          <CardDescription className="flex gap-2">
            {heading.icon}
            <p className="text-(--an-appoint-screen-header-text-color) font-(family-name:--an-appoint-screen-font-family) text-xs font-normal">
              {heading.subtitle}
            </p>
          </CardDescription>
        </CardHeader>
        <CardContent className="px-4">
          <div className="flex flex-col gap-4">
              ...
              {appointScreenData.map((item, index) => (
                <div key={index} className="flex gap-3 items-center">
                  ...
                  <div className="flex flex-col">
                    <p className="text-(--an-appoint-screen-icon-title-color) font-(family-name:--an-appoint-screen-font-family) text-xs font-normal">
                      {item.title}
                    </p>
            ...
            <div className="flex flex-col gap-3">
              <p className="text-(--an-appoint-screen-header-text-color) font-(family-name:--an-appoint-screen-font-family) text-xs font-medium">
                Symptoms
              </p>
              <div className="flex gap-2">
                {symptoms.map((item, index) => (
                  <div
                    key={index}
                    className="flex px-2 py-1 rounded-xs bg-(--an-appoint-screen-symptom-bg-color) text-(--an-appoint-screen-icon-subtitle-color) overflow-ellipsis overflow-hidden font-(family-name:--an-appoint-screen-font-family) text-[10px] font-normal"
                  >
                    {item.title}
                  </div>
                ))}
            ...
            <div className="flex flex-col gap-3 mb-10">
              ...
              <p className="text-(--an-appoint-screen-desc-color) font-(family-name:--an-appoint-screen-font-family) text-xs font-medium">
                {description}
              </p>
            </div>
          </div>
