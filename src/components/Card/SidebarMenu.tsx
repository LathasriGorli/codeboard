...
export function SidebarMenu({ items }: { items: headings[] }) {
  ...
  return (
    ...
    >
      <Sidebar className="flex flex-col h-[calc(100vh-15px)] w-[152px] p-1 itmes-start rounded-sm bg-(--an-menu-background) mt-1.5 ml-1.5 border-none overflow-hidden">
        <SidebarHeader className="bg-white p-1">
          <AlmanaIcon className={"w-18 h-10"} />
        </SidebarHeader>
        <SidebarContent className="bg-white">
          <SidebarGroup className="gap-2 p-0 mt-6">
              {items &&
                items.length > 0 &&
                items.map((item) => (
                  <SidebarMenuItem key={item.id} className="list-none">
                    <SidebarMenuButton
                      asChild
                      className={`px-2 rounded-lg h-6.5 cursor-pointer ${activeId === item.id ? "bg-(--an-sidebar-active-background) hover:bg-(--an-sidebar-active-background) border-1 border-(--an-sidebar-active-text-color) text-(--an-sidebar-active-text-color) hover:text-(--an-sidebar-active-text-color)" : ""}`}
          ...
          <div className="absolute bottom-2 -left-10 !w-48 !h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 bg-gradient-to-tr from-(--an-sidebar-footer-bg) to-(--an-sidebar-footer-bg) blur-xl rotate-[38.723deg] rounded-4xl"></div>
        </SidebarContent>
      </Sidebar>
